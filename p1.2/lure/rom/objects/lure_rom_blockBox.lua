...
function lure.rom.newBlockBoxObject( pParamTable )	
	local self = lure.rom.newBoxObject( {nodeType=1, boxType=1, formatingContext=1, nodeDesc="ROMBlockBoxNode"} )
	...
	self.layout = function()
		...
		if self.formatingContext == 1 then
			lure.rom.layoutEngine.doBlockChildLayouts( self )
		elseif self.formatingContext == 2 then			
			lure.rom.layoutEngine.doInlineChildLayouts( self )
		end								
