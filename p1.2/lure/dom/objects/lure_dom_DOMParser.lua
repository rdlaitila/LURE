...
function lure.dom.DOMParser.new()
	...
	self.openNode = function(pIndex, pType)
		...
		if pType == "tag" then			
			...
			if self.openNodes[nI].hasAttribute('style') then
				if self.openNodes[nI].style ~= nil then
					self.openNodes[nI].style.specificity = lure.dom.computeElementStyleSpecificity( self.openNodes[nI] )
				end
			end
			...
			if string.match(tagContent, "/$") then
				if self.parsedebug then print("parsedebug::tag: '".. tagName .."' is self closing") end
		...
		elseif pType == "CDATASection" then
			...
			if self.parsedebug then
				print("creating child CDATASection Node of Node:".. lastNodeReference.nodeName)			
				print("-CDATA: " .. newNode.data)						
			end
	...
	self.closeNode = function(pIndex)
		--print("closing node at character index '" .. pIndex .. "'")		
		...
		if lure.trim(self.openNodes[nI].tagName:upper()) == lure.trim(tagname):upper() then
			if self.parsedebug then print("parsedebug::closing open node '".. self.openNodes[nI].nodeName.."' Remaining open nodes:") end
			table.remove(self.openNodes, table.getn(self.openNodes))
			for k,v in ipairs(self.openNodes) do
				if self.parsedebug then print("-"..self.openNodes[k].nodeName) end
			end
